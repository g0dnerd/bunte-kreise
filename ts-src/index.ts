...
const coordinates: [[number, number]] =
  await wasm.get_coordinates(jsCoordSpace);
const labels: [[number, String]] = await wasm.get_labels(jsCoordSpace);

const metadata: PointMetadata[] = [];
const data: Data = { labels: [], labelNames: [], projection: [] };
...
const dimensionsToggle =
  document.querySelector<HTMLInputElement>('input[name="3D"]')!;
dimensionsToggle.addEventListener("change", (_: any) => {
...
sequencesToggle.addEventListener("change", (_: any) => {
